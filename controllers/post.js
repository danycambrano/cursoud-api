...
function getPosts(req,res){
    ...
    Post.paginate({},options,(error,postStored)=>{
        if(error){
            res.status(400).send({msg:"Errorl al obtner los posts"});
        }else{
...
function deletePost(req,res){
    ...
    Post.findByIdAndDelete(id,(error)=>{
        if(error){
            res.status(400).send({msg:"El post no se ha podido eliminar"})
        }else{
            res.status(200).send({msg: "El post se ha eliminado con éxito"})
        }
    })
}
